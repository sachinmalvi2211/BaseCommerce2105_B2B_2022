...
public class DemoUsersController
{
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DemoUsersController.class);
    public static final String USER_MAPPER_CONFIG = "firstName,lastName,titleCode,currency(isocode),language(isocode),isPermanentSustainable";
            ...
            final HttpServletRequest httpRequest, final HttpServletResponse httpResponse) throws DuplicateUidException
    {
        ...
        return dataMapper.map(customer, UserWsDTO.class, fields);
    }
    ...
    {
        validate(user, "user", userSignUpDTOValidator);
        final RegisterData registerData = dataMapper
    ...
    /**
     * @deprecated since 2005. Please use {@link DemoUsersController#replaceUser(UserWsDTO)} instead.
     */
            ...
            throws DuplicateUidException
    {
        ...
        dataMapper.map(user, customer, USER_MAPPER_CONFIG, true);
        customerFacade.updateFullProfile(customer);
    }

    /**
     * @deprecated since 2005. Please use {@link DemoUsersController#updateUser(UserWsDTO)} instead.
     */
    @Deprecated(since = "2005", forRemoval = true)
    @Secured({ "ROLE_CUSTOMERGROUP", "ROLE_TRUSTED_CLIENT", "ROLE_CUSTOMERMANAGERGROUP" })
            ...
            throws DuplicateUidException
    {
        final CustomerData customer = customerFacade.getCurrentCustomer();
        LOG.debug("updateUser: userId={}", customer.getUid());
        // Temporary changes for demo purpose , Code can be kept in Facade by extending Facade
        if(null != user.getIsPermanentSustainable())
        {
            CustomerModel customerModel = (CustomerModel) userService.getCurrentUser();
            customerModel.setIsPermanentSustainable(user.getIsPermanentSustainable());
            modelService.save(customerModel);
        }
        dataMapper.map(user, customer, USER_MAPPER_CONFIG, false);
        customerFacade.updateFullProfile(customer);
    }
