...
import de.hybris.platform.order.strategies.calculation.*;
...
public class DemoCalculationService extends AbstractBusinessService implements CalculationService
{
    private static final Logger LOG = Logger.getLogger(DemoCalculationService.class);
    @Resource
    ...
    public void calculate(final AbstractOrderModel order) throws CalculationException
    {
        if (orderRequiresCalculationStrategy.requiresCalculation(order))
        {
            try
            {
                calculateSustainabilityDetails(order);
    ...
    public double getCarbonCreditValue()
    {
        double carbonProductPrice =0;
        CMSSiteModel cmsSite = cmsSiteService.getCurrentSite();
        String carbonCreditValueInSite = "0";
        if(null != cmsSite)
        {
            carbonCreditValueInSite = (null != cmsSite.getCarbonCreditAmount() ? cmsSite.getCarbonCreditAmount()  : "0") ;
        }
        try
        {
    ...
    public void calculateSustainabilityDetails(AbstractOrderModel orderModel)
    {
        ...
        */
        ...
        if(CollectionUtils.isNotEmpty(priceRows))
        {
            ...
            {
                if(priceRow.getCurrency().getIsocode().equalsIgnoreCase(currency.getIsocode()))
                {
        ...
        for(AbstractOrderEntryModel entry : cartEntries)
        {
            ...
            if(!entry.getProduct().getCode().equalsIgnoreCase(getCarbonProduct()))
            {
                ...
                if(checkSustainProduct || isCustomerPermanentSustainMember)
                {
                    ...
                    {
                        double weight = BigDecimal.valueOf(Double.valueOf(carbonFootPrintWeight) * entry.getQuantity()).setScale(2, RoundingMode.HALF_UP).doubleValue();

                        double totalEntryOffsetAmount =   BigDecimal.valueOf(weight * carbonProductPrice).setScale(2, RoundingMode.HALF_UP).doubleValue() ;
                        totalCartItemsWeight = totalCartItemsWeight + weight;
                        entry.setCarbonFootPrintWeight( BigDecimal.valueOf(Double.parseDouble(carbonFootPrintWeight)).setScale(2, RoundingMode.HALF_UP).toString());
                        entry.setCarbonOffsetCredit( BigDecimal.valueOf(carbonProductPrice).setScale(2, RoundingMode.HALF_UP).toString());

                        entry.setEntryTotalSustanabilityValue( BigDecimal.valueOf(totalEntryOffsetAmount).setScale(2, RoundingMode.HALF_UP).toString());
        ...
        for(AbstractOrderEntryModel entry : cartEntries)
        {
            if(entry.getProduct().getCode().equalsIgnoreCase(getCarbonProduct()))
            {
                ...
                entry.setCarbonOffsetCredit(df.format(Double.valueOf(carbonProductPrice)));
                entry.setCarbonFootPrintWeight(df.format(Double.valueOf(totalCartItemsWeight)));
                getModelService().save(entry);
            }
        }
    ...
    {
        AbstractOrderEntryModel carbonProductEntry = null; // = getModelService().create(AbstractOrderEntryModel.class);
        List<AbstractOrderEntryModel> entries = new ArrayList<AbstractOrderEntryModel>();

        Iterator<AbstractOrderEntryModel> itr = abstractOrderModel.getEntries().iterator();
        while(itr.hasNext())
        {
            AbstractOrderEntryModel entry = itr.next();
            if(entry.getProduct().getCode().equals(getCarbonProduct()))
            {
                carbonProductEntry = entry;
            }
            else
            {
                entries.add(entry);
        ...
        // Add CarbonProduct at last in cart
        if(null != carbonProductEntry)
        {
            carbonProductEntry.setEntryNumber(entries.size()+1);
            getModelService().save(carbonProductEntry);
            entries.add(carbonProductEntry);
        }

        Collections.sort(entries, new BeanComparator(AbstractOrderEntryModel.ENTRYNUMBER, new ComparableComparator()));

        List<AbstractOrderEntryModel> finalEntries = new ArrayList<AbstractOrderEntryModel>(entries);
        for (int i = 0; i < entries.size(); i++)
        {
            finalEntries.get(i).setEntryNumber(Integer.valueOf(i));
            getModelService().save(finalEntries.get(i));
        }
        abstractOrderModel.setEntries(finalEntries);
    ...
    public void calculateTotals(final AbstractOrderEntryModel entry, final boolean recalculate)
    {
        if (recalculate || orderRequiresCalculationStrategy.requiresCalculation(entry))
        {
            ...
            if(entry.getProduct().getCode().equalsIgnoreCase(getCarbonProduct()))
            {
                double carbonProductPrice = 0d;
                try {
                    if(StringUtils.isNotEmpty(entry.getCarbonFootPrintWeight()) &&
                            StringUtils.isNotEmpty(entry.getCarbonOffsetCredit()))
                    {
                        carbonProductPrice = Double.valueOf(entry.getCarbonFootPrintWeight()) * Double.valueOf(entry.getCarbonOffsetCredit()) ;
                    }
                }catch(Exception e)
                {
                    LOG.error("Error calculating totals of : "+getCarbonProduct() + " product.");
                }
                entry.setQuantity(1l);
                entry.setTotalPrice(Double.valueOf(carbonProductPrice));
            }
            else {
