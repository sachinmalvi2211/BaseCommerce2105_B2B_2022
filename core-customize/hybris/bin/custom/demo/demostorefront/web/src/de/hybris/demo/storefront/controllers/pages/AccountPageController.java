...
import de.hybris.demo.storefront.forms.DemoUpdateProfileForm;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	public String editProfile(final Model model) throws CMSItemNotFoundException
	{
		...
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final DemoUpdateProfileForm demoUpdateProfileForm = new DemoUpdateProfileForm();

		demoUpdateProfileForm.setTitleCode(customerData.getTitleCode());
		demoUpdateProfileForm.setFirstName(customerData.getFirstName());
		demoUpdateProfileForm.setLastName(customerData.getLastName());
		demoUpdateProfileForm.setPermanentSustainable(customerData.getIsPermanentSustainable());

		model.addAttribute("demoUpdateProfileForm", demoUpdateProfileForm);

		if(null != customerData.getCustomerRewardData())
		{
			CustomerRewardData rewards = customerData.getCustomerRewardData();
			if(StringUtils.isNotEmpty(rewards.getEarnedSustainableRewards()))
			{
				model.addAttribute("rewards",rewards.getEarnedSustainableRewards());
			}
			else {
				model.addAttribute("rewards","0.00");
			}
		}
		else
		{
	...
	@RequireHardLogIn
	public String updateProfile(final DemoUpdateProfileForm demoUpdateProfileForm, final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
	//	validate(demoUpdateProfileForm, bindingResult);

		String returnAction = REDIRECT_TO_UPDATE_PROFILE;
		final CustomerData currentCustomerData = customerFacade.getCurrentCustomer();
		final CustomerData customerData = new CustomerData();
		customerData.setTitleCode(demoUpdateProfileForm.getTitleCode());
		customerData.setFirstName(demoUpdateProfileForm.getFirstName());
		customerData.setLastName(demoUpdateProfileForm.getLastName());
		customerData.setIsPermanentSustainable(demoUpdateProfileForm.getPermanentSustainable());
	...
	private void validate(DemoUpdateProfileForm demoUpdateProfileForm, Errors errors) {
		final String title = demoUpdateProfileForm.getTitleCode();
		final String firstName = demoUpdateProfileForm.getFirstName();
		final String lastName = demoUpdateProfileForm.getLastName();
