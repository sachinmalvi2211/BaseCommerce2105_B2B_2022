        ...
        <li class="item__list--item">
            ...
            <div class="item__info">
                ...
                <div class="item__stock">
                   ...
                   <c:choose>
                           <c:when test="${productCodeHtml == 'CarbonOffsetProduct'}">
                                    <c:if test="${not empty entry.carbonFootPrintWeight}">
                                           <b> Total Cart's
                                           <img src="https://cdn-icons-png.flaticon.com/512/2862/2862791.png" alt="Kg(s)CO2e"
                                           width="30" height="30">
                                            ${entry.carbonFootPrintWeight} Kg(s)CO2e </b>
                                   </c:if>
                           </c:when>
                           <c:otherwise>
                             <c:if test="${not empty entry.product.carbonFootPrintWeight && entry.product.carbonFootPrintWeight != '0'}">
                                    <b> Product's
                                    <img src="https://cdn-icons-png.flaticon.com/512/2862/2862791.png" alt="Kg(s)CO2e"
                                    width="30" height="30">
                                     ${entry.product.carbonFootPrintWeight} Kg(s)CO2e </b>
                            </c:if>
                           </c:otherwise>
                   </c:choose>
            ...
            <c:choose>
                   <c:when test="${productCodeHtml == 'CarbonOffsetProduct'}">
                       ...
                       <format:price priceData="${entry.basePrice}" displayFreeForZero="true"/> / Kg(s)CO2e
                   </c:when>
                   <c:otherwise>
                       <span class="visible-xs visible-sm"><spring:theme code="basket.page.itemPrice"/>: </span>
            ...
            <c:choose>
                   <c:when test="${productCodeHtml == 'CarbonOffsetProduct'}">
                        <!-- <input id="quantity_fixed" name="quantity" class="form-control js-update-entry-quantity-input" type="text" value="${entry.carbonFootPrintWeight}" size="5" disabled> Kg(s)CO2e -->
                        <div><b>${entry.carbonFootPrintWeight} Kg(s)CO2e</b></div>
                   </c:when>
                   <c:otherwise>
                       <c:choose>
                ...
                <c:if test="${entry.product.purchasable}">
                    <c:if test="${not empty entryStock and entryStock ne 'outOfStock'}">
                        <c:if test="${entry.deliveryPointOfService eq null or not entry.product.availableForPickup}">
                            <c:choose>
                                   <c:when test="${productCodeHtml == 'CarbonOffsetProduct'}">
                                       <span class="item__delivery--label">Non-Shippable</span>
                                   </c:when>
                                   <c:otherwise>
            ...
            <div class="item__delivery">
                <c:choose>
                       ...
                       <c:otherwise>
                       <c:choose>
                              <c:when test="${isPermanentSustainable}">
                                     <input type="checkbox" data-toggle="tooltip" title="Help us to make healthy environment ! " id="isSustainableEntry" <c:if test="${entry.isSustainableEntry}">checked=checked</c:if> class="isSustainableEntry-switch-input" disabled>
                              </c:when>
                              <c:otherwise>
                                    <c:url value="cart/updateSustainableEntry/${entry.entryNumber}/${sustainCheckBoxValue}" var="cartUpdateSustainAction" />
                                   <form:form id="updateCartSustainForm${entry.entryNumber}" action="cartUpdateSustainAction" method="post" >
                                       <input type="hidden" name="sustainCheckBoxValue${entry.entryNumber}" value=""/>
                                       <input type="checkbox" title="Help us to make healthy environment !" id="isSustainableEntry${entry.entryNumber}" <c:if test="${entry.isSustainableEntry}">checked=checked</c:if> class="isSustainableEntry-switch-input"
                                       onclick=" var obj = this ;  var form = $(obj).closest('form'); var formId = form.prop('id') ; if (obj.checked) { form.attr('action', 'cart/updateSustainableEntry/${entry.entryNumber}/true');  } else { form.attr('action', 'cart/updateSustainableEntry/${entry.entryNumber}/false') ; }   form.submit();">
                                       <!-- tooltip below -->
                                   </form:form>
                              </c:otherwise>
                       </c:choose>
                        <c:if test="${not empty entry.carbonFootPrintWeight && entry.carbonFootPrintWeight != '0'}">
                                  <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQE7PSXY-dynG_fiW335vePSPvXqrvHmaikUA&usqp=CAU" alt="Thank you for your contribution towards Environment."
                                  width="50" height="50"/>
                         </c:if>
                  </c:otherwise>
               </c:choose>
