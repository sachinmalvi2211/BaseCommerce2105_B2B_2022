...
<li class="item__list--item">
    ...
    <div class="item__info">
        ...
        <div class="item__code">
            ...
            <hr/>
            <c:choose>
                   <c:when test="${fn:escapeXml(orderEntry.product.code) == 'CarbonOffsetProduct'}">
                            <c:if test="${not empty orderEntry.carbonFootPrintWeight}">
                                   <b> Total Order's
                                   <img src="https://cdn-icons-png.flaticon.com/512/2862/2862791.png" alt="Kg(s)CO2e"
                                   width="30" height="30">
                                    ${orderEntry.carbonFootPrintWeight} Kg(s)CO2e </b>
                           </c:if>
                   </c:when>
                   <c:otherwise>
                     <c:if test="${not empty orderEntry.carbonFootPrintWeight && orderEntry.carbonFootPrintWeight != '0'}">
                            <b> Product
                            <img src="https://cdn-icons-png.flaticon.com/512/2862/2862791.png" alt="Kg(s)CO2e"
                            width="30" height="30">
                             ${orderEntry.carbonFootPrintWeight} Kg(s)CO2e </b>
    ...
    <div class="item__price">
        ...
        <c:choose>
                   <c:when test="${orderEntry.product.code == 'CarbonOffsetProduct'}">
                        <order:orderEntryPrice orderEntry="${orderEntry}"/> / Kg(s)CO2e
                   </c:when>
                   <c:otherwise>
    ...
    <div class="item__quantity hidden-xs hidden-sm">
        ...
        <ycommerce:testId code="orderDetails_productQuantity_label">
            ...
            <c:choose>
               <c:when test="${orderEntry.product.code == 'CarbonOffsetProduct'}">
                    <div><b>${orderEntry.carbonFootPrintWeight} Kg(s)CO2e</b></div>
               </c:when>
               <c:otherwise>
                <c:choose>
     ...
     <div  class="item__total hidden-xs hidden-sm">
            <c:if test="${orderEntry.product.code != 'CarbonOffsetProduct'}">
                 <input type="checkbox" disabled id="isSustainableEntry${orderEntry.entryNumber}" <c:if test="${orderEntry.isSustainableEntry}">checked=checked</c:if> class="isSustainableEntry-switch-input" >
            </c:if>
      </div>
